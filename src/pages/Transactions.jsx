...
import { Filter, Search, Edit2, History as HistoryIcon, ArrowDownCircle, ArrowUpCircle, RefreshCcw } from "lucide-react";
...
const Transactions = () => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const params = {
        division: filters.division && filters.division !== "ALL" ? filters.division : null,
  ...
  const categories = [
    "ALL", "Salary", "Business", "Freelance", "Fuel", "Movie", "Food", "Loan", "Medical", "Shopping", "Travel", "Investment", "Rental", "Other",
  ];
    ...
    <div className="space-y-8 pb-10">
      ...
      >
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-widest">Division</label>
          <select
          ...
          >
            <option value="" className="bg-slate-900 text-white">All Divisions</option>
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          >
            {categories.map((c) => (
              <option key={c} value={c === "ALL" ? "" : c} className="bg-slate-900 text-white">{c}</option>
            ))}
          </select>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02]">
                <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Transaction</th>
                <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Division Info</th>
                <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Timestamp</th>
                <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-right">Amount</th>
                <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-center">Manage</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.05]">
              {transactions.map((tx) => (
                <tr key={tx.id} className="group hover:bg-white/[0.02] transition-colors">
                  <td className="p-6">
                    <div className="flex items-center gap-4">
                      <div className={clsx(
                        ...
                        tx.type === "INCOME" ? "bg-teal-500/10 text-teal-500 border border-teal-500/20" :
                          tx.type === "EXPENSE" ? "bg-red-500/10 text-red-500 border border-red-500/20" :
                            "bg-blue-500/10 text-blue-500 border border-blue-500/20"
                      )}>
                        {tx.type === "INCOME" ? <ArrowUpCircle className="w-6 h-6" /> : <ArrowDownCircle className="w-6 h-6" />}
                      </div>
                      <div>
                        <p className="text-base font-black text-white capitalize leading-tight">{tx.category}</p>
                        <p className="text-xs text-slate-500 font-bold">{tx.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-6">
                  ...
                  <td className="p-6">
                    <p className="text-sm font-bold text-slate-300">{format(new Date(tx.timestamp), "MMM dd, yyyy")}</p>
                    <p className="text-[10px] text-slate-500 font-medium uppercase">{format(new Date(tx.timestamp), "hh:mm a")}</p>
                  </td>
                  <td className={clsx(
                    "p-6 text-xl font-black text-right tracking-tight",
                    tx.type === "INCOME" ? "text-teal-400" : tx.type === "EXPENSE" ? "text-red-400" : "text-blue-400"
                  )}>
                    ...
                    <div className="flex flex-col items-center gap-2">
                      {isEditable(tx.timestamp) ? (
                        ...
                        <div className="flex flex-col items-center opacity-30 grayscale items-center gap-1">
                          <div className="p-3 text-slate-500"><Edit2 className="w-4 h-4" /></div>
                  ...
                  <td colSpan="5" className="py-24 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">
                        {loading ? "Decrypting Ledger..." : "No records found in this vector"}
                      </p>
                    </div>
                  </td>
                </tr>
